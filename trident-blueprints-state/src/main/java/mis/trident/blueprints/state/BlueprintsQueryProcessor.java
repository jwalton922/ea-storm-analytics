...
//        }
        log.info("BatchRetrieve found " + values.size() + " values");
    ...
    public void execute(TridentTuple tuple, Object object, TridentCollector collector) {
        if (tuple != null) {
            ...
            log.info("BlueprintsQueryProcessor execute called on null tuple");
        }
        
        if(object != null){
           // log.debug("BlueprintsQueryProcessor execute called on object: "+ object.toString());
        } else {
            log.info("BlueprintsQueryProcessor execute called on null OBJECT");
            return;
        }
